...
## GET `/api/1/energy_sites/{site_id}/calendar_history?kind=energy&period={period_keyword}&end_date={date_and_time}`

Retrieve the energy generation statistics, in kWh, for a specified period ending on a specified date.  
...
| :--------- | :-------------------------------------------- | :-------------------------------------------------------------- |
| `kind`     | `energy`                                      | The value "energy" selects energy statistics, in kWh.           | 
| `period`   | `day`, `week`, `month`, `year`, or `lifetime` | The time span for statistics to cover.                          |
| `end_date` | yyyy-mm-ddThh:mm:ss-hh:mm                     | Specifies the last day for which the power statistics are retrieved.<br>Example: `2022-04-29T14:15:00-07:00` |
